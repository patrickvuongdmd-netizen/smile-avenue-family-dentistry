...
const TrustTicker = () => {
  ...
  return (
    <div className="flex overflow-hidden relative border-b border-border/40 transition-all duration-300 [[data-menu-open]_&]:h-0 [[data-menu-open]_&]:overflow-hidden [[data-menu-open]_&]:opacity-0" style={{ backgroundColor: '#DCDCDC' }}>
      ...
      <div className="relative overflow-hidden flex-1">
        ...
        <div className="flex animate-ticker whitespace-nowrap">
          {doubled.map((text, i) => (
            <span
              key={i}
              className="inline-flex items-center shrink-0 px-3 sm:px-5 py-1 sm:py-2 text-[10px] sm:text-xs tracking-wide font-semibold font-sans text-primary"
            >
              {text}
              <span className="ml-3 sm:ml-5 text-primary/30">·</span>
            </span>
          ))}
        </div>
      </div>
